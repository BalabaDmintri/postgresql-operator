...
from ..helpers import (
    ...
    get_password,
    get_unit_address,
    run_command_on_unit, scale_application, FIRST_DATABASE_RELATION_NAME, tag_storage,
)
...
async def test_deploy_zero_units(ops_test: OpsTest):
    ...
    await ops_test.model.applications[APPLICATION_NAME].add_unit(attach_storage=[tag_storage(primary_storage)])
    logger.info("checking whether writes are increasing")
    await are_writes_increasing(ops_test)
